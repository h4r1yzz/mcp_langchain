...
uv run streamlit run combined_app.py
```
...
2. **Streamlit Application** (`combined_app.py`) - Alternative interface using Streamlit
...
- Connects to the MCP server to access LAS file analysis tools
...
An MCP (Model Context Protocol) server that provides tools for LAS file analysis:

- Exposes MCP tools (`get_file_metadata`, `get_visualization`, `get_ascii_data`) that can be called by the AI agent
- Parses and analyzes LAS file data
- Generates visualizations of well log data using Plotly
- Executes Python code generated by the AI to answer queries on LAS data
- Manages file paths and provides LAS file analysis capabilities
